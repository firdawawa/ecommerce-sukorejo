            ...
            <div class="bg-white px-2 py-4 mx-5 -mt-2 mb-4 rounded-xl h-full">
                ...
                <div class="mt-10 grid grid-cols-[17rem_1fr] divide-x-2">
                    ...
                    <div class="block w-56 m-6 -mt-2">
                        <RouterLink to="/bumdes/overview/month" class="block text-lg text-font font-normal rounded-xl pl-4 pr-18 py-5 hover:bg-primary hover:text-white hover:font-bold">Bulan Ini</RouterLink>
                        <RouterLink to="/bumdes/overview" class="block text-lg text-font font-normal rounded-xl pl-4 pr-18 py-5 hover:bg-primary hover:text-white hover:font-bold">Seluruh Data</RouterLink>
                    </div>
                    <!-- CONTENT -->
                    <div class="pl-12 items-center">
                        ...
                        <div class="flex justify-between">
                            <div class="flex gap-8 ml-10 mt-4 border-2 rounded-xl w-fit px-8 py-4">
                                    ...
                                    <div class="text-secondary font-bold text-4xl">{{ totalStores }}</div>
                                </div>
                            <div class="flex gap-4 ml-10 mr-8 border-2 rounded-xl w-fit px-8 py-4">
                                <select v-model="selectedYear" @change="fetchStoreData" class="border rounded px-4 py-2">
                        ...
                        <h2 class="font-bold text-2xl mt-16 text-font">Statistik Penjualan</h2>
                        <hr class="h-px my-6 mr-8 bg-gray-400 border-1">
                        <div class="flex justify-between">
                            <div class="flex gap-8 ml-10 mt-4 border-2 rounded-xl w-fit px-8 py-4">
                                <div class="text-font">Jumlah Produk</div>
                                <div class="text-secondary font-bold text-4xl">{{ totalProducts }}</div>
                            </div>
                            <div class="flex gap-4 ml-10 mr-8 border-2 rounded-xl w-fit px-8 py-4">
                                <select v-model="selectedYear" @change="fetchProductData" class="border rounded px-4 py-2">
                                    <option v-for="year in years" :key="year" :value="year">
                                        {{ year }}
                                    </option>
                                </select>
                        ...
                        <div class="w-[90%] mx-auto mt-8 h-96">
                            <BarChart v-if="chartDataProducts" :chartData="chartDataProducts" />
                            <p v-else class="text-gray-500">Memuat data...</p>
                        </div>

                        <h2 class="font-bold text-2xl mt-16 text-font">Statistik Pendapatan</h2>
                        <hr class="h-px my-6 mr-8 bg-gray-400 border-1">
                        <div class="flex justify-between">
                            <div class="flex gap-8 ml-10 mt-4 border-2 rounded-xl w-fit px-8 py-4">
                                <div class="text-font">Jumlah Pendapatan</div>
                                <div class="text-secondary font-bold text-4xl">{{ totalSales }}</div>
                            </div>
                            <div class="flex gap-4 ml-10 mr-8 border-2 rounded-xl w-fit px-8 py-4">
                        ...
                        <div class="flex justify-between">
                            <div class="flex gap-8 ml-10 mt-4 border-2 rounded-xl w-fit px-8 py-4">
                                ...
                                <div class="text-secondary font-bold text-4xl">{{ salesDataQuantity }}</div>
                            </div>
                            <div class="flex gap-4 ml-10 mr-8 border-2 rounded-xl w-fit px-8 py-4">
                                <select v-model="selectedYear" @change="getMonthlySalesQuantity" class="border rounded px-4 py-2">
                                    <option v-for="year in years" :key="year" :value="year">
                                        {{ year }}
                                    </option>
                                </select>
                        ...
                        <div class="w-[90%] mx-auto mt-8 h-96">
                            <BarChart v-if="chartDataSalesQuantity" :chartData="chartDataSalesQuantity" />
                            <p v-else class="text-gray-500">Memuat data...</p>
                        </div>

                        <h2 class="font-bold text-2xl mt-16 text-font">Statistik Kategori Toko</h2>
                        <hr class="h-px my-6 mr-8 bg-gray-400 border-1">
                        <div class="mx-auto mt-8 h-96">
                            <PieChart v-if="chartDataCategory" :chartData="chartDataCategory" />
                            <p v-else class="text-gray-500">Memuat data...</p>
                        </div>
                            ...
                            <div>
                                ...
                                <ul class="space-y-4">
                                    ...
                                    >
                                        ...
                                        <p class="text-md text-gray-500">{{ product.store_name }}</p>
                                        <p class="text-lg text-green-600">Terjual: {{ product.total_sold }}</p>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <ul class="space-y-4">
                                    ...
                                    >
                                        ...
                                        <p class="text-lg text-green-600">Total Penjualan: {{ store.total_sold }}</p>
                                        </div>
                                    </li>
                                </ul>
                            </div>
...
import { Chart as ChartJS, Title, Tooltip, Legend, BarElement, ArcElement, CategoryScale, LinearScale, BarController, PieController } from 'chart.js';
import BumdesSidebarNavigation from '../../../components/bumdes/SidebarNavigation.vue';

ChartJS.register(Title, Tooltip, Legend, BarElement, ArcElement, CategoryScale, LinearScale, BarController, PieController);
...
export default {
    ...
    setup() {
        ...
        const fetchStoreData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/statistics/all-stores', {
        ...
        const fetchProductData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/statistics/all-products', {
                    params: { year: selectedYear.value }
                });
        ...
        const fetchSalesData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/statistics/all-income', {
                    params: { year: selectedYear.value }
                });
        ...
        const getMonthlySalesQuantity = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/statistics/all-sales', {
                    params: { year: selectedYear.value }
                });
                ...
                salesDataQuantity.value = formattedData.reduce((acc, val) => acc + val, 0);
        ...
        const fetchStoresByCategory = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/statistics/stores-category');
                storesByCategory.value = response.data;

                // Buat array kategori dan jumlah toko
                const labels = storesByCategory.value.map(item => `Kategori ${item.category_id}`);
                const data = storesByCategory.value.map(item => item.total_stores);

                chartDataCategory.value = {
                    labels: ['Makanan dan Minuman', 'Kerajinan', 'Fashion', 'Agribisnis', 'Kecantikan', 'Peternakan', 'Otomotif', 'Elektronik', 'Toserba', 'Lainnya'],
                    datasets: [
                        {
                            label: "Jumlah Toko per Kategori",
                            backgroundColor: [
                                "#FF6384", "#36A2EB", "#FFCE56", "#4CAF50", "#E26A4A",
                                "#8E44AD", "#2ECC71", "#F39C12", "#3498DB", "#D35400"
                            ],
                            data: data
                        }
                    ]
        ...
        const fetchBestSellingProducts = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:8000/api/statistics/best-selling");
        ...
        const fetchBestSellingStores = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:8000/api/statistics/best-selling-stores");
