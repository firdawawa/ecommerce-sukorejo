                    ...
                    <div class="pl-12 items-center">
                        ...
                        <div>
                            <div class="flex mt-6 justify-between">
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 ml-8">
                                    <div v-for="(file, index) in produk.files" :key="index" class="m-1">
                                        <img :src="getImageUrl(file)" id="fotoproduk" alt="Foto Produk" class="h-36 w-36 rounded-lg object-cover">
                                    </div>
                                </div>
                                <div class="ml-auto text-right mr-8">
                                    <span class="text-font font-normal">Ditambahkan pada {{ formatDate(produk.created_at) }}</span>
                                    <div class="flex mt-4 items-center gap-2 justify-end">
                                        <img src="../../../assets/images/icon-cart-2.png" alt="">
                                        <div class="flex gap-1">
                                            <span id="totalsell" class="text-font font-normal">{{ produk.total_sold }} </span>
...
export default {
    ...
    methods: {
        ...
        async getProduk(idProduct) {
                try {
                    const res = await axios.get(`http://127.0.0.1:8000/api/store/${this.idStore}/product/${idProduct}`, {
                        headers: {
