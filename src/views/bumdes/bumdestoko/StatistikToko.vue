...
export default {
    ...
    methods: {
        ...
        async fetchSalesData() {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/statistic-dailysales/${this.idStore}`, {
                    params: { month: this.selectedMonth, year: this.selectedYear }
                });
                this.totalSold = response.data.reduce((sum, item) => sum + Number(item.total_sold), 0);
                this.chartData = {
                    labels: response.data.map(item => item.date),
                    datasets: [{ label: "Total Produk Terjual", backgroundColor: "#4A21C4", data: response.data.map(item => item.total_sold) }]
                };
            } catch (error) { console.error(error); }
        },
        async fetchIncomeData() {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/statistic-dailyincome/${this.idStore}`, {
                    params: { month: this.selectedMonth, year: this.selectedYear }
                });
                this.totalIncome = response.data.reduce((sum, item) => sum + Number(item.total_income), 0);
                this.incomeChartData = {
                    labels: response.data.map(item => item.date),
                    datasets: [{ label: "Total Pendapatan", backgroundColor: "#21C45A", data: response.data.map(item => item.total_income) }]
                };
            } catch (error) { console.error(error); }
        }   
    }
