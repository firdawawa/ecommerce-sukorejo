					...
					<div class="pl-8 items-center">
                        ...
                        <div class="flex mt-6 justify-between">
                            <div>
                                ...
                                <span id="timestamppesanan" class="text-md text-gray-500"  v-if="order">{{ formatDate(order?.created_at) }}</span>
                            </div>
                            <div>
                                <div id="orderstatus" class="text-font font-bold text-xl mr-10" v-if="order && order.status">
                                    {{ order.status.label }}
                                </div>
                            </div>
                        </div>
							...
							<table class="w-full text-md text-left rtl:text-right text-font">
								...
								<tbody>
									<tr v-for="(item, index) in mergedOrderData" :key="item.product_id" class="bg-white border-b hover:bg-gray-100">
										...
										<td id="price" class="p-6" v-if="order && order.invoice">Rp {{ this.order.invoice.subtotal_price }}</td>
									</tr>
								</tbody>
							</table>
						</div>
                        <h2 class="font-bold text-2xl text-font mt-12">Informasi Pembeli</h2>
                        <hr class="h-px my-4 mr-8 bg-gray-400 border-1">
                        <div class="flex gap-10">
                            <div class="w-1/2">
                                <div class="text-font font-semibold text-lg">Pengiriman</div>
                                <div id="buyername" class="text-font font-normal text-md my-4" v-if="order && order.order_address">{{ maskText(this.order.order_address.name) }}</div>
                                <div id="buyerphone" class="text-font font-normal text-md my-4" v-if="order && order.order_address">{{ maskText(this.order.order_address.phone) }}</div>
                                <div id="buyeraddress" class="text-font font-normal text-md my-4" v-if="order && order.order_address">{{ maskText(this.order.order_address.detail_address) }}, {{ maskText(subDistrict.subDistrictName) }}, {{ maskText(district) }}, {{ maskText(this.order.order_address.postal_code) }}</div>
                            </div>
                            <div class="w-1/2">
                        ...
                        <h2 class="font-bold text-2xl text-font mt-12">Ringkasan Pendapatan</h2>
                        <hr class="h-px my-4 mr-8 bg-gray-400 border-1">
                        <div>
                            <div class="flex justify-between mr-12 my-4">
                                <div class="font-bold text-lg text-font">Subtotal Pendapatan</div>
                                <div id="subprice" class="font-normal text-md text-font" v-if="order && order.invoice">Rp {{ this.order.invoice.total_price }}</div>
                            </div>
                            <div class="flex justify-between mr-12 my-4">
                            ...
                            <div class="flex justify-between mr-12 my-4">
                                <div class="font-bold text-2xl text-secondary">Total Pendapatan</div>
                        ...
                        <button v-if="order && order.status.id == 2" @click="orderDelivered"
	...
	methods: {
        ...
        async fetchOrderData(order_id) {
                try {
                    ...
                    const response = await axios.get(`http://127.0.0.1:8000/api/dashboard/store/${this.idStore}/order/${order_id}`, {
                        headers: {
        ...
        async orderDelivered() {
            try {
                ...
                if (!this.idStore) {
                    alert("idStore tidak ditemukan. Pastikan Anda sudah login sebagai seller.");
                    return;
                }
                console.log("Token:", token);
                const response = await axios.put(`http://127.0.0.1:8000/api/dashboard/store/${this.idStore}/order/${this.order_id}/status`, {},
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
