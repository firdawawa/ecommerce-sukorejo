...
export default {
	...
	methods: {
		...
		handleFileUpload(event) {
			...
			if (files.length > 0) {
				...
				if (validFiles.length < files.length) {
					alert(
						'Beberapa file tidak diunggah karena melebihi batas ukuran 100MB.',
					);
				}
			...
			axios
				...
				.then(res => {
					if (res.data.data) {
						...
						this.selectedFileName = res.data.data.files
							? 'File telah diunggah'
							: '';
					}
				})
		...
		updateProduk() {
			...
			if (
				Array.isArray(this.model.produk.files) &&
				this.model.produk.files.length > 0
			) {
			...
			axios
				.post(
					`https://api.isnunas.my.id/api/dashboard/store/${this.idStore}/product/${this.produkId}`,
					formData,
					{
						headers: {
							'Content-Type': 'multipart/form-data',
