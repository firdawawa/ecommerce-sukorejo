	...
	setup() {
		...
		const fetchSalesData = async () => {
			try {
				...
				const response = await axios.get(
					`https://api.isnunas.my.id/api/statistic-dailysales/${idStore}`,
					{
		...
		const fetchIncomeData = async () => {
			try {
				...
				const response = await axios.get(
					`https://api.isnunas.my.id/api/statistic-dailyincome/${idStore}`,
					{
						headers: { Authorization: `Bearer ${token}` },
						params: { month: selectedMonth.value, year: selectedYear.value },
					},
				...
				totalIncome.value = income.reduce(
		...
		const fetchBestSellingProducts = async () => {
			try {
				const response = await axios.get(
					`https://api.isnunas.my.id/api/statistic-best-selling/${idStore}`,
				);
