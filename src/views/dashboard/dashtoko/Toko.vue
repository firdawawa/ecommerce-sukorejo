		...
		<div class="flex-1 ml-1/4">
			<div class="bg-white px-2 py-4 mx-5 -mt-2 mb-4 rounded-xl h-full">
				<div class="flex justify-between">
					...
					<div class="mr-8 mt-2">
						<RouterLink
							:to="{ path: '/dashboard/profil/toko/' + idStore + '/edit' }"
						>
							<button
				...
				<div class="mt-12 grid grid-cols-[17rem_1fr] divide-x-2">
					...
					<div v-else class="pl-12 items-center">
						...
						<div class="flex justify-center">
							<img
								:src="getImageUrl(store.shopProfileImg)"
								id="photoprofile"
								alt="photo-profile"
						...
						<div class="flex">
							...
							<div class="text-font text-md font-normal pr-6 w-[800px]">
								<h4 id="address" class="py-2"
									>{{ user.addressDetail }}, {{ subDistrict.subDistrictName }},
									{{ district }}</h4
								>
							</div>
						</div>
...
export default {
	...
	methods: {
		...
		getStore(idStore) {
			axios
				.get(`https://api.isnunas.my.id/api/store/${idStore}`, {
					headers: {
	...
	async beforeMount() {
		try {
			...
			const response = await axios.get(
				'https://api.isnunas.my.id/api/dashboard/store/check',
				{
					headers: {
