		...
		<div class="flex-1 ml-1/4">
			<div class="bg-white px-2 py-4 mx-5 -mt-2 mb-4 rounded-xl h-full">
				<div class="flex justify-between">
					...
					<div class="mr-8 mt-2">
						<button
							@click="navigateTo('/dashboard/iklan/add')"
					...
					<table class="w-full text-md text-left rtl:text-right text-font">
						<thead class="text-lg font-semibold text-white bg-primary">
							<tr>
								...
								<th scope="col" class="px-6 py-4"> Nama Iklan </th>
								<th scope="col" class="px-6 py-4"> Tautan </th>
						...
						<tbody>
							...
							>
								<th scope="row" class="px-6 py-2 font-medium">
									<RouterLink
										:to="{ path: '/dashboard/iklan/' + iklan.id + '/detail' }"
								...
								<td class="px-6 py-2">
									...
									>
										...
										/>
										<span class="font-md text-font font-semibold">{{
											iklan.title
										}}</span>
									</RouterLink>
								</td>
								...
								<td class="pl-6 py-2 text-center">
									<div class="flex gap-2">
										...
										>
											<button
												class="flex gap-4 bg-primary content-center justify-center align-middle py-4 px-4 rounded-full"
											>
												<img
		...
		async getIklan() {
			try {
				const res = await axios.get(
					`https://api.isnunas.my.id/api/dashboard/store/${this.idStore}/advertisement`,
					{
						headers: {
