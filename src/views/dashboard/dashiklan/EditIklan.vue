				...
				<div class="flex justify-between">
					...
					<div class="mr-8 mt-2">
						<button
							v-if="iklanId"
							@click="deleteIklan()"
							class="flex gap-4 bg-red-500 content-center justify-center align-middle h-12 py-2 px-10 rounded-xl"
						>
							<img
								src="../../../assets/images/icon-delete.png"
								alt="edit"
								class="content-center h-5"
							/>
						...
						<form class="" action="#">
							...
							<div>
								...
								<button
									...
									class="w-full text-primary bg-purple-200 border-dotted border-2 border-primary hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-normal rounded-lg text-lg px-5 py-8 text-center cursor-pointer"
								>
								...
								<span class="text-sm font-light text-gray-400"
									...
									atau .pdf dengan ukuran maksimal 100MB.</span
								>
							</div>
						</form>
					</div>
				...
				<div>
					<form action="">
						<div class="mr-4">
							...
							<textarea
								type="text"
								v-model="model.iklan.description"
								name="desc"
								id="desc"
								rows="6"
...
export default {
	...
	methods: {
			...
			axios
				...
				.then(res => {
					if (res.data.data) {
						...
						this.selectedFileName = res.data.data.banner
							? 'File telah diunggah'
							: '';
					}
				})
		...
		updateIklan() {
			if (
				!this.model.iklan.title ||
				!this.model.iklan.advurl ||
				!this.model.iklan.description
			) {
			...
			axios
				.post(
					`https://api.isnunas.my.id/api/dashboard/store/${this.idStore}/advertisement/${this.iklanId}`,
					formData,
					{
						headers: {
