		...
		<div class="mt-28 mx-48 mb-12">
			<div class="bg-white w-full h-full rounded-2xl p-10">
				...
				<div class="flex w-full">
					<div class="w-1/3">
						<img
							v-if="produk?.files?.length > 0"
							:src="getImageUrl(produk.files[0])"
							alt=""
							class="rounded-2xl aspect-square w-full object-cover"
						/>
					</div>
					<div class="ml-20 mt-4 w-2/3 mr-12">
						...
						<div class="flex mt-6 w-full justify-between">
							<div class="text-secondary text-5xl font-bold">{{
								produk.price
							}}</div>
						...
						<div
							class="flex border bg-white border-gray-300 px-10 py-6 rounded-xl mt-10"
						>
							<img
								:src="getStoreImageUrl(store.shopProfileImg)"
								alt=""
								class="h-28 w-28 object-cover rounded-full border border-gray-200"
							/>
							<div class="block ml-10 w-full mt-2">
								<div class="flex w-full justify-between">
									...
									<RouterLink
										:to="{ path: '/toko/' + idStore + '/detail' }"
										class="flex gap-2"
									>
										<div
											class="text-font text-xl font-semibold border border-secondary px-7 py-2 rounded-xl"
											>Kunjungi Toko</div
										>
									</RouterLink>
								</div>
								<div class="flex justify-between mt-2">
									<div class="text-light text-lg text-gray-400">{{
						...
						<div class="flex mt-6 gap-8 items-center">
							<button
								...
								class="flex bg-secondary rounded-xl px-9 py-5 gap-4 items-center"
							>
								<div class="text-white text-3xl font-bold -mt-2">+</div>
								<div class="text-white text-xl">Tambah ke Keranjang</div>
							</button>
							<div
								class="flex items-center gap-10 rounded-lg border border-gray-400 px-3 h-12"
							>
								<button>
									<img
				...
				<div class="py-6 mt-10">
					...
					<div class="border border-gray-300 rounded-xl p-4 mt-4">{{
						produk.description
					}}</div>
				</div>
...
export default {
	...
	methods: {
		...
		getAllProduk() {
			axios
				.get(`https://api.isnunas.my.id/api/product/${this.produkId}`, {
					headers: {
		...
		async getProduk() {
			try {
				const res = await axios.get(
					`https://api.isnunas.my.id/api/store/${this.idStore}/product/${this.produkId}`,
					{
						headers: {
		...
		getTotalSold(idStore) {
			axios
				.get(`https://api.isnunas.my.id/api/store/${idStore}/totalsold`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${localStorage.getItem('token')}`,
					},
				...
				.catch(error => {
					console.error(
						`Error fetching total sold for store ${idStore}:`,
						error,
					);
				});
		},
		getTotalProduct() {
			axios
				.get(
					`https://api.isnunas.my.id/api/store/${this.idStore}/total-products`,
					{
						headers: {
							'Content-Type': 'application/json',
							Authorization: `Bearer ${localStorage.getItem('token')}`,
				...
				.catch(error => {
					console.error(
						`Error fetching total products for store ${this.idStore}:`,
						error,
					);
				});
		},
