    ...
    <div>
        ...
        <div class="mt-28 mx-48 mb-12">
            ...
            <div class="relative bg-white w-full h-full rounded-2xl py-6 px-6">
                ...
                <div class="border border-gray-400 rounded-2xl w-full h-full pt-6 pb-16 px-32 my-8">
                    <div class="flex justify-center">
                        <img :src="getImageUrl(advertisement.banner)" alt="" class="rounded-xl h-96">
                    </div>
                    <div class="flex font-bold text-3xl text-font justify-center mt-8">{{ advertisement.title }}</div>
                    <div class="flex justify-between mt-12">
                        <div v-if="advertisement.store">
                            <div class="flex gap-4 items-center">
                                <img src="../../assets/images/icon-store.png" alt="" class="h-8">
                                <div class="text-font text-xl">{{ advertisement.store.storeName }}</div>
                            </div>
                            <div class="text-font text-xl mt-5 ml-12">
                                {{ categoryMap[advertisement.store.idCategory] || "Kategori Tidak Diketahui" }}
                            </div>
                        </div>
                        <div>
                            <div class="flex gap-4 items-center">
                                <img src="../../assets/images/icon-link.png" alt="" class="h-12">
                                <div class="text-font text-xl">{{ advertisement.advurl }}</div>
                            </div>
                            <div class="text-font text-xl mt-2 ml-16">Ditambahkan pada {{ formatDate(advertisement.created_at) }}</div>
                        </div>
                    </div>
                    <div class="font-bold text-font text-2xl mt-6">Deskripsi</div>
                    <div class="text-font text-xl mt-4 text-justify">{{ advertisement.description }}</div>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getAdvertisement(advId) {    
			axios.get(`http://127.0.0.1:8000/api/advertisement/${advId}`, {
				headers: {
