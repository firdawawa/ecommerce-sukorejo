		...
		<div class="mt-28 mx-48 mb-12">
			<div class="bg-white w-full h-full rounded-2xl mt-4 py-8">
				...
				<div
					...
					class="grid grid-cols-2 px-8 gap-4 gap-y-0"
				>
					<div
						v-for="(store, index) in stores"
						:key="index"
						class="relative py-6 w-full"
					>
						<RouterLink
							:to="{ path: '/toko/' + store.id + '/detail' }"
							class="flex w-full border bg-white border-gray-300 px-8 py-6 rounded-xl"
						>
							<img
								:src="getStoreImageUrl(store.shopProfileImg)"
								alt=""
								class="h-40 w-40 object-cover rounded-full border border-gray-200"
							/>
							<div class="block ml-6 w-full mt-2">
								<div class="flex w-full justify-between">
									<div class="font-bold text-font text-3xl">{{
										store.storeName
									}}</div>
								...
								<div class="flex justify-between mt-4">
									<div class="text-light text-lg text-font">{{
										getCategoryName(store.idCategory)
									}}</div>
									<div class="flex gap-2">
										<img
											src="../../assets/images/icon-user-check.png"
											alt=""
											class="h-7 w-7"
										/>
										<div class="text-light text-lg text-font">{{
								...
								<div class="flex mt-6 justify-between">
									<div class="flex gap-2 items-center justify-center">
										<img
											...
											class="h-8 w-8"
										/>
										<div class="font-medium text-xl text-font"
											>{{ store.products_count }} Produk</div
										>
									</div>
									<div class="flex gap-2 items-center justify-center">
										<img
											src="../../assets/images/icon-cart-3.png"
											alt=""
											class="h-8 w-8"
										/>
										<div class="font-medium text-xl text-font"
											>{{
												store.totalSold !== undefined ? store.totalSold : '0'
											}}
											Terjual</div
										>
				...
				<div v-else class="flex justify-center px-8 my-8 text-lg text-gray-500"
					>Tidak ada toko yang sesuai dengan pencarian.</div
				>
				...
				<div
					v-if="ads?.length > 0"
					class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 my-4"
				>
					<div v-for="(ad, index) in ads" :key="index" class="w-full px-8">
						<RouterLink
							:to="{ path: '/iklan/' + ad.id + '/detail' }"
							class="w-full border bg-white border-gray-300 px-4 py-4 rounded-xl shadow-sm flex flex-col"
						>
							<img
								:src="getAdImageUrl(ad.banner)"
								alt=""
								class="h-70 w-full object-cover rounded-xl"
							/>
							<div class="flex flex-col items-center mt-4">
								<div class="font-bold text-font text-xl text-center">{{
									ad.title
								}}</div>
							</div>
							...
							<div class="text-gray-500 text-md mt-1">{{
								categoryMap[ad.store.idCategory] || 'Kategori Tidak Diketahui'
							}}</div>
						</RouterLink>
					</div>
				</div>
				<div v-else class="flex justify-center px-8 my-8 text-lg text-gray-500"
					>Tidak ada iklan yang sesuai dengan pencarian.</div
				>
				...
				>
					<div
						v-for="(product, getAll) in products"
						:key="getAll"
						class="w-full px-8"
					>
						<RouterLink
							:to="{ path: '/produk/' + product.id + '/detail' }"
							class="border border-gray-400 rounded-3xl w-full relative flex flex-col"
						>
							<img
								:src="getProductImageUrl(product)"
								alt=""
								class="w-full rounded-t-3xl h-72 object-cover"
							/>
							<button
								@click.stop.prevent="addToCart(product.id)"
								class="absolute top-60 right-4"
							>
								<div
									class="flex h-20 w-20 bg-secondary rounded-full items-center content-center justify-center"
								>
									<img
										src="../../assets/images/icon-cart-produk.png"
										alt=""
							...
							<div class="mt-4 px-7">
								<div class="text-2xl font-bold text-font">{{
									product.name
								}}</div>
								<div class="text-gray-400 mt-1 font-light">{{
									categoryMap[product.store.idCategory] ||
								...
								<div class="flex justify-between my-6">
									...
									<div class="font-light text-gray-400"
										>{{ product.total_sold }} Terjual</div
									>
								</div>
							</div>
						</RouterLink>
					...
					>
						<div
							class="bg-white px-10 py-24 rounded-lg shadow-lg w-1/3 text-center"
						>
							<h2 class="text-2xl font-bold text-red-600">Anda belum login!</h2>
							<p class="text-font mt-12 text-lg"
							...
							<div class="mt-10 flex justify-center gap-4">
								<button
									...
									class="text-lg font-semibold px-12 py-4 bg-blue-500 text-white rounded-lg"
									>Login</button
								>
								<button
									@click="showLoginModal = false"
									class="text-lg font-semibold px-12 py-4 bg-gray-400 text-white rounded-lg"
									>Tutup</button
								>
							</div>
						</div>
				...
				<div v-else class="flex justify-center px-8 my-8 text-lg text-gray-500"
					>Tidak ada produk yang sesuai dengan pencarian.</div
				>
			</div>
		</div>
	</div>
	...
	methods: {
		...
		async fetchSearchResults() {
			...
			try {
				const response = await axios.get(
					`https://api.isnunas.my.id/api/search`,
					{
		...
		addToCart(productId) {
			...
			if (!token) {
				...
				axios
					.post(
						'https://api.isnunas.my.id/api/cart/add',
						{
							idProduct: productId,
							quantity: 1,
						},
		...
		getStoreImageUrl(image) {
			return image
				? `https://api.isnunas.my.id/storage/${image}`
				: placeholderImage;
		},
		getAdImageUrl(image) {
			return image
		...
		getProductImageUrl(product) {
			return product.first_image && product.first_image.file
				? `https://api.isnunas.my.id/storage/${product.first_image.file}`
				: placeholderImage;
		},
		getCategoryName(idCategory) {
