				...
				<div class="bg-white w-1/5 h-fit fixed rounded-2xl p-10 pb-28">
					<div class="profile content-center mr-20 ml-4">
						<RouterLink to="/profil" class="flex content-center gap-6">
							<img
								v-if="user && user.data"
								:src="getImage(this.user.data.profileImg)"
								alt="profile picture"
				...
				<div class="w-3/4 ml-1/4">
					<div class="bg-white h-fit rounded-2xl pb-6 pt-2 px-14 ml-4">
						...
						<div class="flex justify-between items-center mt-10 mx-8">
							...
							>
							<div
								class="mr-8 text-2xl text-primary font-bold"
						...
						<div>
							<div class="flex justify-between mr-12 my-4">
								...
								<div id="itemquantity" class="font-normal text-md text-font"
									>x{{ this.orderData.quantity }}</div
								>
							</div>
							<div class="flex justify-between mr-12 my-4">
								<div class="font-semibold text-lg text-font"
									>Subtotal Harga</div
								>
								<div
									id="subprice"
									...
									>Rp {{ this.order.invoice.subtotal_price }}</div
								>
							</div>
							<div class="flex justify-between mr-12 my-4">
								...
								<div
									...
									class="font-normal text-md text-font"
									v-if="order && order.invoice"
									>Rp {{ this.order.invoice.discount }}</div
								>
							</div>
							<div class="flex justify-between mr-12 my-4">
							...
							<div class="flex justify-between mr-12 my-4">
								<div class="font-semibold text-2xl text-secondary"
									>Total Harga</div
								>
								<div
									id="totalprice"
						...
						<div class="flex gap-24">
							<div class="w-1/2">
								...
								<div
									...
									>{{ this.order.order_address.name }}</div
								>
								<div
									id="buyerphone"
									class="text-font font-normal text-md my-4"
									v-if="order && order.order_address"
									>{{ this.order.order_address.phone }}</div
								>
								<div
									id="buyeraddress"
									class="text-font font-normal text-md my-4"
									v-if="order && order.order_address"
									>{{ this.order.order_address.detail_address }},
									{{ subDistrict.subDistrictName }}, {{ district }},
									{{ this.order.order_address.postal_code }}</div
								>
							</div>
							<div class="w-1/2">
		...
		async fetchOrderData(order_id) {
			try {
				...
				const response = await axios.get(
					`https://api.isnunas.my.id/api/order/${order_id}`,
					{
						headers: {
