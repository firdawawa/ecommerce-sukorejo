<template>
    <div>
        ...
        <div class="mt-28 mx-48 mb-12">
            ...
            <!-- KATEGORI -->
            <div class="relative bg-white w-full h-fit rounded-2xl py-6 px-6">
                <div class="text-4xl font-bold text-font">Toko</div>
            ...
            <!-- LIST TOKO -->
            <div class="bg-white w-full h-full rounded-2xl mt-4">
                <div class="grid grid-cols-2 px-8 gap-4 gap-y-0">
                    <div v-for="(store, index) in store" :key="index" class="relative py-6 w-full">
                        <RouterLink :to="{ path: '/toko/' + store.id + '/detail'}" class="flex w-full border bg-white border-gray-300 px-8 py-6 rounded-xl">
                            <img :src="getImageUrl(store.shopProfileImg)" alt="" class="h-40 w-40 object-cover rounded-full border border-gray-200">
    ...
    methods: {
        ...
        getTotalSold(idStore) {
            ...
            .then(res => {
                ...
                // Cari index store yang sesuai
                const storeIndex = this.store.findIndex(store => store.id === idStore);
                if (storeIndex !== -1) {
                    this.store[storeIndex].totalSold = res.data.total_sold;
                }
            })
            .catch(error => {
