			...
			<div class="bg-white w-full h-full rounded-2xl p-10">
						...
						<form class="" action="#">
							<div>
								...
								<input
									...
									name="name"
									id="name"
									class="border border-gray-400 text-font rounded-lg focus:ring-primary-600 focus:border-primary-600 block h-12 p-2.5 w-full bg-white placeholder-font"
									placeholder=""
									required=""
								/>
							</div>
					...
					<div class="columns-2 mt-4 w-full">
						<form class="" action="#">
							<div>
								...
								>
								<input
									v-model="model.address.detail_address"
									type="text"
									name="addressDetail"
									id="addressDetail"
									class="border border-gray-400 text-font rounded-lg focus:ring-primary-600 focus:border-primary-600 block h-12 p-2.5 w-full bg-white placeholder-font"
				...
				<div v-if="selectedItems.length">
					<div
						...
						:key="item.id"
						class="border border-gray-400 w-full rounded-xl py-6 my-4"
					>
						<div class="flex justify-between items-center mx-12">
							<div class="flex gap-8 items-center">
								<img
									:src="getStoreImageUrl(item.product.store.shopProfileImg)"
									alt=""
						...
						<div class="flex mx-12 mt-4 items-center justify-between mb-8">
							<div class="flex items-center">
								<img
									v-if="item.product?.files?.length > 0"
								...
								<div class="ml-10">
									...
									<div class="text-lg text-font font-normal mt-2"
										>x{{ item.quantity }}</div
									>
								</div>
							</div>
							<div class="flex gap-64">
								<div>
									<div class="text-font text-lg font-semibold"
										>Rp {{ item.price }}</div
									>
								</div>
								<div>
									<div class="text-primary text-2xl font-semibold mr-12"
										>Rp {{ item.price * item.quantity }}</div
									>
								</div>
							</div>
						</div>
				...
				<div class="columns-2 mt-8">
					<div>
						...
						<div>
							...
							>
							<select
								v-model="model.order.id"
								id="deliverymethod"
								class="border border-gray-400 text-font placeholder-gray-400 text-md rounded-lg focus:ring-primary-600 focus:border-primary-500 w-full block p-2.5"
							>
								<option
									v-for="order in order_delivery"
									:key="order.id"
									:value="order.id"
								>
									{{ order.label }}
				...
				<div>
					<div class="flex justify-between mr-12 my-4">
						...
						<div id="itemquantity" class="font-normal text-md text-font"
							>x{{
					...
					<div class="flex justify-between mr-12 my-4">
						...
						<div id="subprice" class="font-normal text-md text-font"
							>Rp
							{{
					...
					<div class="flex justify-between mr-12 my-4">
						...
						<div id="totalprice" class="font-bold text-4xl text-secondary"
							>Rp
							{{
								selectedItems.reduce(
									(sum, item) => sum + item.price * item.quantity,
				...
				<button
					type="submit"
					@click="checkout"
					class="w-full text-white bg-secondary hover:bg-primary border focus:ring-4 focus:outline-none focus:ring-primary-300 font-bold rounded-lg text-xl px-5 py-5 mt-10 mb-4 text-center"
					>Buat Pesanan</button
				>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		async fetchCartData(cartItemId) {
			try {
				...
				const response = await axios.get(
					`https://api.isnunas.my.id/api/cart/item/${cartItemId}`,
					{
						headers: {
				...
				this.selectedItems = Array.isArray(response.data.data)
					? response.data.data
					: [response.data.data];
				...
				console.log(
					'Shop Profile Image:',
					response.data.data?.product?.store?.shopProfileImg,
				);
			} catch (error) {
				console.error(
		...
		fetchDeliveryMethod() {
			axios
				.get(`https://api.isnunas.my.id/api/order-delivery`, {
					headers: {
		...
		getStoreImageUrl(filePath) {
			if (filePath) {
				const imageUrl = `https://api.isnunas.my.id/storage/${filePath}`;
		...
		checkout() {
			...
			axios
				.post('https://api.isnunas.my.id/api/order', payload, {
					headers: {
						Authorization: `Bearer ${localStorage.getItem('token')}`,
						'Content-Type': 'application/json',
					},
		...
		deleteCartItem(cartItemId) {
			axios
				.delete(`https://api.isnunas.my.id/api/cart/remove/${cartItemId}`, {
					headers: {
						Authorization: `Bearer ${localStorage.getItem('token')}`,
					},
				})
